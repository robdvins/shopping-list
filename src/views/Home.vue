...
const { mutate: createList } = useMutation(CREATE_LIST)
...
async function createNewList() {
  ...
  const q = await createList({ id: uid(), name: list.name })

  router.push({
    name: 'ShoppingList',
    params: {
      id: q?.data.response.id
    }
  })
}
</script>
...
<template>
  <div class="mx-auto w-full max-w-md">
    <div class="mt-32 flex flex-col space-y-10">
